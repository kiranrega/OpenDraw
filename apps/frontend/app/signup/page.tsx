...
import { Eye, EyeOff, Palette, User, Mail, Lock, ArrowRight, CheckCircle , X} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-4 selection:bg-gray-800">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none opacity-[0.03]">
        <svg className="absolute -top-24 -right-24 w-96 h-96 text-white transform rotate-12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.5">
          <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
        </svg>
        <svg className="absolute -bottom-24 -left-24 w-96 h-96 text-white transform -rotate-12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.5">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center mb-8">
          ...
          <h1 className="text-4xl font-light text-white mb-2 tracking-tight">Create Account</h1>
          <p className="text-gray-400">Join thousands of visual thinkers today.</p>
        </motion.div>
          ...
          {errorMessage || fieldErrors ? (
            <div className="mb-6 bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl flex flex-col gap-2">
              <div className="flex items-start justify-between">
                <div className="text-sm font-semibold">{errorMessage ?? 'Registration failed'}</div>
                <button
                  type="button"
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-500 group-focus-within:text-white transition-colors" />
                <input
                  type="text"
                  value={formData.username}
                  onChange={(e) => handleInputChange('username', e.target.value)}
                  className={`w-full pl-12 pr-4 py-3 bg-gray-950 border rounded-xl text-white placeholder-gray-600 focus:outline-none transition-all ${fieldErrors?.username ? 'border-red-500/50' : 'border-gray-800 focus:border-gray-600'}`}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full pl-12 pr-4 py-3 bg-gray-950 border rounded-xl text-white placeholder-gray-600 focus:outline-none transition-all ${fieldErrors?.email ? 'border-red-500/50' : 'border-gray-800 focus:border-gray-600'}`}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full pl-12 pr-12 py-3 bg-gray-950 border rounded-xl text-white placeholder-gray-600 focus:outline-none transition-all ${fieldErrors?.password ? 'border-red-500/50' : 'border-gray-800 focus:border-gray-600'}`}
                  placeholder="••••••••"
                  required
                />
                <button
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <CheckCircle className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-500 group-focus-within:text-white transition-colors" />
                <input
                  type={showConfirmPassword ? 'text' : 'password'}
                  value={formData.confirmPassword}
                  onChange={(e) => handleInputChange('confirmPassword', e.target.value)}
                  className={`w-full pl-12 pr-12 py-3 bg-gray-950 border rounded-xl text-white placeholder-gray-600 focus:outline-none transition-all ${fieldErrors?.confirmPassword ? 'border-red-500/50' : 'border-gray-800 focus:border-gray-600'}`}
                  placeholder="••••••••"
                  required
                />
                <button
            ...
            {errors.length > 0 && (
              <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                {errors.map((error, index) => (
                  <p key={index} className="text-red-400 text-xs font-medium text-center">{error}</p>
                ))}
              </div>
            )}
